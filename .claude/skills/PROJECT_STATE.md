...
| **Account isolation (R-014, new 2026-09-18)** | **BROKEN, confirmed by the account owner.** The legacy `MMT_TradePannel_Pro` EA is live-trading on the same account as Stage 2, 2 pairs open — discovered by chance in a pasted log, not by any check this project had. `35_1000_USD_LIVE_TEST_PLAN.md` §7's "no other positions" precondition does not currently hold. This EA is independently known-unreliable from this project's own research (408 `OpenLeg FAIL` in under 50 minutes). Recommended: close its positions and detach it. Not yet resolved as of this entry — check `RISK_REGISTER.md` R-014 before treating this account as isolated. |
