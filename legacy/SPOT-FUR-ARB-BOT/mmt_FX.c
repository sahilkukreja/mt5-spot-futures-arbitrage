...
//|                           v2.81                                   |
//|  Top-Down Control (NEW LOGIC):                                     |
//|   - Top inputs: LOT | OAG | CAG (ABS gap target)                   |
//|   - One button: "OPEN / SCHEDULE"                                 |
//|       * If OAG == 0  -> OPEN immediately at market                |
//|       * If OAG != 0  -> SCHEDULE entry waiting for gap hit        |
//|   - If CAG == 0 -> target disabled (never auto-close on target)   |
//|   - Idle pairs hidden: only ACTIVE or SCHEDULED rows shown        |
//|   - Per-row: UPDATE (sets CAG from top), CLOSE / CANCEL           |
//|   - OPENGAP shown from fills (entry prices), never used for CAG    |
//|   - FIX: CAG is NEVER derived from any open gap / delta logic      |
//|          (prevents "19.81" surprise target)                        |
//|   - Header: Terminal Total P/L vs Visible List Sum P/L            |
//+------------------------------------------------------------------+
#property strict
...
// No rounding, no forced decimals. This prevents weird values from formatting.
...
bool PairIsFull(int pairIdx){ return (FindTicket(pairIdx,"S1")!=0 && FindTicket(pairIdx,"S2")!=0); }

// STATUS: 0=Idle, 1=Active, 2=Scheduled
int GetPairStatus(int p)
{
  if(PairIsFull(p)) return 1;
  if(GvGetD(p,"STATUS",0.0) == 2.0) return 2;
  return 0;
}
...
int NextFreePairSlot()
{
  for(int p=1;p<=g_maxPairs;p++)
    if(FindTicket(p,"S1")==0 && FindTicket(p,"S2")==0 && GetPairStatus(p)!=2)
      return p;
  return -1;
}
...
double GetVisibleListSumPL()
{
  ...
  for(int i=0;i<g_visibleN;i++)
  {
    ...
    if(GetPairStatus(p)==1) s += GetPairPL_ProfitOnly(p);
  }
  return s;
}
...
double GetVisibleListSumLots()
{
  ...
  {
    int p=g_visiblePairs[i];
    if(GetPairStatus(p)==1) s += GetPairLot_PairSize(p);
    else if(GetPairStatus(p)==2) s += GvGetD(p,"SCHED_LOT",0.0);
  }
  return s;
}
...
bool OpenLeg(string sym,ENUM_ORDER_TYPE type,double lot,string cmt)
{
  ...
  if(!ok)
    LogMsg(StringFormat("OpenLeg FAIL %s %s lot=%s ret=%d (%s)",
                        sym,(type==ORDER_TYPE_BUY?"BUY":"SELL"),DoubleToString(lot,2),
...
bool OpenPair(int pairIdx,double lot)
{
  ...
  // opening makes it active; clear schedule flag if any
...
void ApplyAutoRules()
{
  ...
  for(int p=1;p<=g_maxPairs;p++)
  {
    ...
    // SCHEDULED: trigger open when OAG hit
    if(status==2)
    {
      double oag = GvGetD(p,"OAG",0.0);
      double lot = GvGetD(p,"SCHED_LOT",0.01);

      // SELL_ONLY: trigger if cg >= oag
      // BUY_ONLY : trigger if cg <= oag
      bool trigger = (InpOpenDirection==DIR_SELL_ONLY) ? (cg >= oag) : (cg <= oag);

      if(trigger)
      {
        LogMsg(StringFormat("SCHEDULED TRIGGER HIT: Pair %d cg=%s OAG=%s", p, F2(cg), F2(oag)));
        OpenPair(p, lot);
      }
      continue;
    }

    // ACTIVE: close on CAG hit (if CAG != 0)
    if(status==1)
    {
      ...
      if(TargetHit_Abs(cg,tg) && profit_ok)
      {
        LogMsg(StringFormat("AutoClose CAG hit P=%d pl=%s cg=%s target=%s", p, F2(pl), F2(cg), F2(tg)));
        ClosePair(p);
      }
    }
  }
...
// Line 1
CreateLabel(g_prefix+"TITLE_1", x0, y, 
            "Enter LOT | OAG | CAG", 
            UI_FontBase-3, InpColTitleText);

  y += LineH()+15;
CreateLabel(g_prefix+"TITLE_2", x0, y, 
            "then click OPEN/SCHEDULE (OAG=0 opens now, OAG!=0 schedules)", 
            UI_FontBase-3, InpColTitleText);
  ...
  // Rows (indexed by visible row r = 0..N-1)
  for(int r=0;r<g_visibleN;r++)
  {
    ...
    int bx = UI_X + UI_W - pad - (RowBtnW()+RowGap()+RowBtnW());
...
int OnInit()
{
  ...
  LogMsg("Initialized v2.81. MaxPairs=" + IntegerToString(g_maxPairs));
...
void OnChartEvent(const int id,const long &lparam,const double &dparam,const string &sparam)
{
  ...
  if(sparam == g_prefix+"BTN_OPEN_SCHED")
  {
    ...
    // IMPORTANT FIX: target is EXACTLY top CAG (abs). Never computed/shifted.
    GvSetD(p,"TARGET", cag);

    if(oag==0.0)
    {
      // OPEN NOW
      // ensure no schedule flags
      GvSetD(p,"STATUS",0.0);
      GvSetD(p,"OAG",0.0);
      GvSetD(p,"SCHED_LOT",0.0);
    ...
    else
    {
      ...
      LogMsg(StringFormat("SCHEDULE: slot=%d lot=%s OAG=%s CAG=%s", p, F2(lot), F2(oag), F2(cag)));
    }
    return;
  }
  ...
  {
    for(int p=1;p<=g_maxPairs;p++)
      if(GetPairStatus(p)==1) ClosePair(p);
    return;
  }
  ...
  for(int r=0;r<g_visibleN;r++)
  {
    ...
    if(sparam == upd)
    {
      ...
      GvSetD(p,"TARGET", cag);
      LogMsg(StringFormat("UPDATE CAG: pair=%d newCAG=%s", p, F2(cag)));
      return;
    }
