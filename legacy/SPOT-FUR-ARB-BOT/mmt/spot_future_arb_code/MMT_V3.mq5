...
bool OpenPair(int pairIdx,double lot)
{
  ...
  // ALWAYS reset timers when opening a new pair on this slot
  ...
  // clear OAG schedule value so UI doesn't show stale on LIVE
...
void ClosePair(int pairIdx)
{
  ...
  // Reset timers after closing (manual or auto)
  ResetHitTimers(pairIdx);
}
...
void ApplyAutoRules()
{
  ...
  for(int p=1;p<=g_maxPairs;p++)
  {
    ...
    if(status==2)
    {
      ...
      bool hit = (InpOpenDirection==DIR_SELL_ONLY) ? (cg >= oag) : (cg <= oag);
      ...
      bool active = (GvGetD(p,"OAG_HIT_ACTIVE",0.0) >= 0.5);
      ulong since = (ulong)GvGetD(p,"OAG_HIT_SINCE_MS",0.0);

      if(hit)
      {
        if(!active)
        {
          // first time becoming true
          GvSetD(p,"OAG_HIT_ACTIVE",1.0);
          GvSetD(p,"OAG_HIT_SINCE_MS",(double)now);
          // (optional) log only once to avoid spam
          // LogMsg(StringFormat("OAG HIT START: P=%d cg=%s OAG=%s", p, F2(cg), F2(oag)));
        }
        else
        {
          ulong elapsed = (now >= since ? (now - since) : 0);
          if((int)elapsed >= InpOagConfirmMs)
          {
            ...
            OpenPair(p, lot); // OpenPair resets timers + clears schedule
          }
        }
      }
      else
      {
        ...
        if(active)
        {
          GvSetD(p,"OAG_HIT_ACTIVE",0.0);
          GvSetD(p,"OAG_HIT_SINCE_MS",0.0);
          // LogMsg(StringFormat("OAG HIT RESET: P=%d", p));
        }
      }
      continue;
    }
    ...
    if(status==1)
    {
      ...
      bool active = (GvGetD(p,"CAG_HIT_ACTIVE",0.0) >= 0.5);
      ulong since = (ulong)GvGetD(p,"CAG_HIT_SINCE_MS",0.0);
      ...
      if(hit)
      {
        if(!active)
        {
          ...
          // LogMsg(StringFormat("CAG HIT START: P=%d cg=%s target=%s", p, F2(cg), F2(tg)));
        }
        else
        {
          ulong elapsed = (now >= since ? (now - since) : 0);
          if((int)elapsed >= InpCagConfirmMs)
          {
            ...
            if(profit_ok)
            {
              ...
              ClosePair(p); // ClosePair resets timers
            }
            else
            {
              // Profit gate blocked: keep timer running? safer to reset so it must "re-hold" again
              GvSetD(p,"CAG_HIT_ACTIVE",0.0);
              GvSetD(p,"CAG_HIT_SINCE_MS",0.0);
      ...
      else
      {
        if(active)
        {
          GvSetD(p,"CAG_HIT_ACTIVE",0.0);
          GvSetD(p,"CAG_HIT_SINCE_MS",0.0);
          // LogMsg(StringFormat("CAG HIT RESET: P=%d", p));
        }
      }
    }
  }
...
void CancelSchedule(int p)
{
  ...
  GvSetD(p,"OAG",0.0);
  GvSetD(p,"SCHED_LOT",0.0);
  // keep TARGET? safer to clear so next use starts clean
  GvSetD(p,"TARGET",0.0);
