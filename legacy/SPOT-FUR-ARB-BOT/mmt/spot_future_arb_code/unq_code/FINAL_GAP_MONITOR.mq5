...
input color  UI_TextColor       = clrDarkBlue;
input color  UI_HeaderTextColor = clrDarkBlue;

input color  UI_BorderColor     = clrSilver;
input int    UI_BorderAlpha     = 220;

input color  UI_GridColor       = clrDarkGreen;
...
{
  int lineH = UI_FontSize + 11;
  int lines = 0;

  lines += 2; // title
  if(UI_ShowSymbols)     lines += 2;
  if(UI_ShowMidPrices)   lines += 2; // now prices row
  lines += 1; // gap
  if(UI_ShowCurrentHour) lines += 2;
...
bool RenderPanel(datetime now)
{
  ...
  int padL = 12;
  int padT = 30;
  int lineH = UI_FontSize + 20 + UI_FontSize * 0.8 ;

  int x = panelX + padL;
  int y = panelY + padT;

  string dirTxt = (InpOpenDirection==DIR_SELL_ONLY) ? "SELL_ONLY (Bid1-Ask2)" : "BUY_ONLY (Ask1-Bid2)\r";
  ...
  EnsureLabel(UI_PREFIX+"TH0", tableX + 15,               tableY + 2, "Window", UI_HeaderTextColor);
  EnsureLabel(UI_PREFIX+"TH1", tableX + col0 + col0 + 8,        tableY + 2, "Max",    UI_HeaderTextColor);
